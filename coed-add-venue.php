...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Step 2: Insert into coed_reservation with venue_sub
    $stmt = $conn->prepare("INSERT INTO coed_reservation (venue_sub, date_of_request, name, college_name, event_activity, event_date, time_of_event) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    if ($stmt->execute()) {
        ...
        if (isset($_POST['venue_requests'])) {
            ...
            $venue_stmt = $conn->prepare("INSERT INTO coed_venue_request (reservation_id, venue_name) VALUES (?, ?)");
            if (!$venue_stmt) {
           ...
           // Function to toggle the visibility of additional requests
            ...
            function showSuccessAlert() {
                Swal.fire({
                    ...
                    window.location.href = "coed-venue.php"; // Redirect to the dashboard or desired page
                });
            }
