...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM coed_tor WHERE id = $eventId";
    ...
    $detailsSql = "SELECT * FROM coed_food WHERE coed_tor_id = $eventId";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Get posted data for coed_event_form
    ...
    if (isset($_GET['id'])) {
        ...
        $updateEventSql = "UPDATE coed_tor SET 
        ...
        if ($conn->query($updateEventSql) === TRUE) {
            ...
            $deleteDetailsSql = "DELETE FROM coed_food WHERE coed_tor_id = $eventId";
            ...
            foreach ($event_dates as $index => $date) {
                ...
                $insertDetailsSql = "INSERT INTO coed_food (coed_tor_id, event_date, event_title, meal_type, menu, total_meals, total_usage) VALUES ($eventId, '$date', '$title', '$mealType', '$menuItem', $meals, $usage)";
                if (!$conn->query($insertDetailsSql)) {
                    $_SESSION['error'] = 'Error inserting event detail: ' . $conn->error;
                    header("Location: coed-edit-tor.php?id=$eventId");
                    exit;
                }
            }
            ...
            header("Location: coed-edit-tor.php?id=$eventId"); // Redirect back to the edit page
            exit;
        } else {
           ...
           // Set initial event count based on existing event sections
            ...
            function showSuccessAlert(message) {
                Swal.fire({
                    ...
                    window.location.href = "coed-tor.php"; // Redirect to coed-tor.php
                });
            }
