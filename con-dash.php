...
$sql = "SELECT id, task_description, created_at, con_status FROM con_notifications ORDER BY created_at DESC";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_notification'])) {
    ...
    $sql = "DELETE FROM con_notifications WHERE id = ?";
    ...
    while ($row = $result_due_date->fetch_assoc()) {
        ...
        if (!in_array($taskId, $_SESSION['notified_tasks'])) {
            ...
            if (!empty($notificationMessage)) {
                ...
                $insertNotification = $conn->prepare("INSERT INTO con_notifications (department, task_description, con_status, created_at) VALUES (?, ?, 'unread', NOW())");
                $department = 'CON'; // Set department explicitly (or dynamically if needed)
    ...
    <body>
        ...
        <div class="content">
            ...
            <div class="container">
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['task_id'])) {
                    ...
                    $stmt = $conn->prepare("SELECT task_description FROM con_tasks WHERE id = ?");
                    ...
                    $stmt = $conn->prepare("UPDATE con_tasks SET status = 'done' WHERE id = ?");
                    $stmt->bind_param("i", $task_id);
                ...
                $sql = "SELECT id, task_description, due_date FROM con_tasks WHERE status = 'pending' ORDER BY created_at DESC";
                $result = $conn->query($sql);
                ...
                $sql_done = "SELECT id, task_description, due_date FROM con_tasks WHERE status = 'done' ORDER BY created_at DESC";
        ...
        <script>
            ...
            function markNotificationsAsRead() {
                fetch('con-mark-notifications-read.php', {
                    method: 'POST',
                })
           ...
           // Function to delete a notification
            function deleteNotification(notificationId) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        fetch('con-delete-notification.php', {
                            method: 'POST',
                            headers: {
            ...
            document.addEventListener("DOMContentLoaded", () => {
                ...
                function fetchNotifications() {
                    ...
                    fetch(`con-fetch_task.php?last_fetch_time=${encodeURIComponent(lastFetchTime)}`)
                        .then(response => response.json())
                        .then(data => {
                            const notificationCountElement = document.querySelector('.notification-count');
                            ...
                            if (data.notifications.length > 0) {
                                ...
                                notificationCountElement.textContent = data.notifications.filter(n => n.con_status === 'unread').length;
                            } else {
