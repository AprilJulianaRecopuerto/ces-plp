...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare to insert into cas_details table
    $detailsStmt = $conn->prepare("INSERT INTO cas_details (semester, district, barangay) VALUES (?, ?, ?)");
    if (!$detailsStmt) {
        die("Prepare failed: " . $conn->error);
    }
    $detailsStmt->bind_param("sss", $semester, $district, $barangay);

    // Execute the statement for cas_details
    if ($detailsStmt->execute()) {
        ...
        foreach ($event_titles as $index => $event_title) {
            ...
            $total_expenses_query = $conn->prepare("SELECT SUM(expenses) AS total_expenses FROM cas_budget WHERE details_id = ?");
            ...
            // Prepare to insert into cas_budget table
            $budgetStmt = $conn->prepare("INSERT INTO cas_budget (details_id, event_title, total_budget, expenses, remaining_budget) VALUES (?, ?, ?, ?, ?)");
            if (!$budgetStmt) {
                die("Prepare failed: " . $conn->error);
            }
            // Insert the current event budget entry into cas_budget table
        ...
        if ($conn_mov->query($notification_sql) === TRUE) {
            ...
            if ($result->num_rows > 0) {
                ...
                try {
                    ...
                    header("Location: cas-add-budget.php");
                    exit;
                }
            } else {
    ...
    header("Location: cas-add-budget.php"); // Adjust this to your needs
    exit();
}

// Fetch the profile picture from the colleges table in user_registration
$conn_profile = new mysqli($db_host, $db_user, $db_pass, $dbname_user_registration);
