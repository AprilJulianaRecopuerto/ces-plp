...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM cas_reservation WHERE id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $sqlVenue = "SELECT venue_name FROM cas_venue_request WHERE reservation_id = ?";
        ...
        $sqlAdditional = "SELECT additional_request, quantity FROM cas_addedrequest WHERE reservation_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_GET['id'])) {
        ...
        $updateReservationSql = "UPDATE cas_reservation SET 
        ...
        if ($conn->query($updateReservationSql) === TRUE) {
            ...
            $deleteVenuesSql = "DELETE FROM cas_venue_request WHERE reservation_id = $reservationId";
            ...
            foreach ($newVenueRequests as $venue) {
                if (!empty($venue)) {
                    $insertVenueSql = "INSERT INTO cas_venue_request (reservation_id, venue_name) 
            ...
            $deleteAdditionalSql = "DELETE FROM cas_addedrequest WHERE reservation_id = $reservationId";
            ...
            if (!empty($newAdditionalRequests) && is_array($newAdditionalRequests)) {
                foreach ($newAdditionalRequests as $request) {
                    if (!empty($request)) {
                        ...
                        $insertAdditionalSql = "INSERT INTO cas_addedrequest (reservation_id, additional_request, quantity) VALUES (?, ?, ?)";
        ...
        Swal.fire({
            ...
            function showSuccessAlert(message) {
                Swal.fire({
                    ...
                    window.location.href = "cas-venue.php"; // Redirect to cas-resource.php
                });
            }
