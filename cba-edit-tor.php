...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM cba_tor WHERE id = $eventId";
    ...
    $detailsSql = "SELECT * FROM cba_food WHERE cba_tor_id = $eventId";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Get posted data for cba_event_form
    ...
    if (isset($_GET['id'])) {
        ...
        $updateEventSql = "UPDATE cba_tor SET 
        ...
        if ($conn->query($updateEventSql) === TRUE) {
            ...
            $deleteDetailsSql = "DELETE FROM cba_food WHERE cba_tor_id = $eventId";
            ...
            foreach ($event_dates as $index => $date) {
                ...
                $insertDetailsSql = "INSERT INTO cba_food (cba_tor_id, event_date, event_title, meal_type, menu, total_meals, total_usage) VALUES ($eventId, '$date', '$title', '$mealType', '$menuItem', $meals, $usage)";
                if (!$conn->query($insertDetailsSql)) {
                    $_SESSION['error'] = 'Error inserting event detail: ' . $conn->error;
                    header("Location: cba-edit-tor.php?id=$eventId");
                    exit;
                }
            }
            ...
            header("Location: cba-edit-tor.php?id=$eventId"); // Redirect back to the edit page
            exit;
        } else {
           ...
           // Set initial event count based on existing event sections
            ...
            function showSuccessAlert(message) {
                Swal.fire({
                    ...
                    window.location.href = "cba-tor.php"; // Redirect to cba-tor.php
                });
            }
