...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($currentProject && (
        ...
        header("Location: cas-editproj.php?id=" . $projectId); // Redirect back with project ID
        exit();
    }
    ...
    if ($stmt->execute()) {
        ...
        header("Location: cas-editproj.php?id=" . $projectId);
        exit();
    } else {
        $_SESSION['error'] = 'Failed to update project: ' . $stmt->error;
        header("Location: cas-editproj.php?id=" . $projectId); // Redirect with project ID
        exit();
    }
...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        header("Location: cas-projlist.php"); // Redirect to project list if no ID is found
        exit();
    }
} else {
    $_SESSION['error'] = 'No project ID specified.';
    header("Location: cas-projlist.php"); // Redirect to project list if no ID is specified
    exit();
}
                ...
                <form action="" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="lead_person" id="lead_person" required onchange="toggleOthersInput()">
                            <option value="CAS Coordinator" <?php echo ($project['lead_person'] == 'CAS Coordinator') ? 'selected' : ''; ?>>CAS Coordinator</option>
                            <option value="Others" <?php echo ($project['lead_person'] != 'CAS Coordinator') ? 'selected' : ''; ?>>Others (Please specify)</option>
                        </select>
                    </div>

                    <!-- Conditional input field for "Others" -->
                    <div class="form-group" id="othersInputGroup" style="display: <?php echo ($project['lead_person'] != 'CAS Coordinator') ? 'block' : 'none'; ?>;">
                        <input type="text" name="other_lead_person" id="other_lead_person" value="<?php echo htmlspecialchars($project['lead_person'] != 'CAS Coordinator' ? $project['lead_person'] : ''); ?>">
                    </div>
            ...
            document.addEventListener('DOMContentLoaded', (event) => {
                ...
                function showSuccessAlert(message) {
                    Swal.fire({
                        ...
                        window.location.href = "cas-projlist.php"; // Redirect to the project list
                    });
                }
                ...
                function showWarningAlert(message) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            window.location.href = 'cas-projlist.php';
                        }
