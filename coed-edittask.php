...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT project_name, department, due_date, letter_request, act_plan, termof_ref, venue_reserve FROM coed WHERE id = ?");
    ...
    $stmt = $conn->prepare("UPDATE coed SET 
        project_name=?, 
        department=?, 
        due_date=?, 
        letter_request=?, 
    ...
    if ($stmt->execute()) {
        ...
        header("Location: coed-edittask.php?id=" . $projectId); // Redirect with project ID
        exit();
    } else {
...
if (isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM coed WHERE id = ?");
    if ($stmt === false) {
        die("Prepare failed: " . $conn->error);
    }
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION['error'] = 'Project not found.';
        header("Location: coed-task.php"); // Redirect to project list if no ID is found
        exit();
    }
} else {
    $_SESSION['error'] = 'No project ID specified.';
    header("Location: coed-task.php"); // Redirect to project list if no ID is specified
    exit();
}
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['letter_request'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $letterFileExtension === 'pdf' ? 'coed-task-fileviewer.php?file=' . urlencode($project['letter_request']) : $letterFilePath; ?>" 
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['act_plan'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $actPlanFileExtension === 'pdf' ? 'coed-task-fileviewer.php?file=' . urlencode($project['act_plan']) : $actPlanFilePath; ?>" 
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['termof_ref'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $termOfRefFileExtension === 'pdf' ? 'coed-task-fileviewer.php?file=' . urlencode($project['termof_ref']) : $termOfRefFilePath; ?>" 
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['venue_reserve'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $venueReserveFileExtension === 'pdf' ? 'coed-task-fileviewer.php?file=' . urlencode($project['venue_reserve']) : $venueReserveFilePath; ?>" 
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            function showSuccessAlert(message) {
                Swal.fire({
                    ...
                    window.location.href = "coed-task.php"; // Redirect to the project list
                });
            }
