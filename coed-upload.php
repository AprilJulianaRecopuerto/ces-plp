...
$uploadDir = 'movuploads/coed-mov/' . $folder_name . '/';
if (!is_dir($uploadDir)) {
    mkdir($uploadDir, 0777, true);
}

// Define the recycle bin directory
$recycleBinDir = 'movuploads/coed-recycle/';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'delete_image') {
        ...
        $newImagePath = $recycleBinDir . basename($imagePath); // Move to the coed-recycle
            ...
            <?php if (isset($_SESSION['action_taken'])): ?>
                <?php if (isset($_SESSION['folder_rename_success'])): ?>
                    showSuccessAlert('<?php echo addslashes($_SESSION['folder_rename_success']); ?>').then(() => {
                        window.location.href = "coed-upload.php?folder=<?php echo urlencode($folder_name); ?>"; // Redirect after closing the alert
                    });
