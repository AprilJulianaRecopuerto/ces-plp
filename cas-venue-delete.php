...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['reservation_id'])) {
    ...
    $countQuery = "
        SELECT 
            (SELECT COUNT(*) FROM cas_venue_request WHERE reservation_id = ?) AS venue_count,
            (SELECT COUNT(*) FROM cas_addedrequest WHERE reservation_id = ?) AS request_count
    ";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reservation_id'])) {
    ...
    if ($venue_name === 'all' && $request_name === 'all') {
        // Delete all venues
        $deleteAllVenuesQuery = "DELETE FROM cas_venue_request WHERE reservation_id = ?";
        ...
        $deleteAllRequestsQuery = "DELETE FROM cas_addedrequest WHERE reservation_id = ?";
        ...
        if ($success) {
            $deleteReservationQuery = "DELETE FROM cas_reservation WHERE id = ?";
        ...
        $deleteSingleVenueQuery = "DELETE FROM cas_venue_request WHERE reservation_id = ? LIMIT 1";
        ...
        $deleteSingleRequestQuery = "DELETE FROM cas_addedrequest WHERE reservation_id = ? LIMIT 1";
        ...
        if ($venue_name) {
            $deleteVenueQuery = "DELETE FROM cas_venue_request WHERE reservation_id = ? AND venue_name = ?";
        ...
        if ($request_name) {
            $deleteRequestQuery = "DELETE FROM cas_addedrequest WHERE reservation_id = ? AND additional_request = ?";
