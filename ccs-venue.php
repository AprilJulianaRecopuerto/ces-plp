...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO ccs_reservation (date_of_request, name, college_name, event_activity, date_of_event, time_of_event, remarks) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    if ($stmt->execute()) {
        ...
        if (isset($_POST['venue_requests'])) {
            $venue_stmt = $conn->prepare("INSERT INTO ccs_venue_request (reservation_id, venue_requests) VALUES (?, ?)");
            if (!$venue_stmt) {
                $_SESSION['error'] = "Preparation failed for venue request: " . $conn->error;
                header("Location: ccs-venue.php"); // Redirect back to the form page
                exit;
            }
    ...
    <body>
        ...
        <div class="content-venue">
            <div class="button-container">
               <button onclick="logAndRedirect('Add Venue', 'ccs-venue-list.php')">Facilities Reservation Form</button>
            </div>
            ...
            <div class="data-details">
                ...
                <tbody>
                <?php
                  $requestSql = "SELECT * FROM ccs_reservation ORDER BY id DESC";
                        ...
                        if ($requestResult && $requestResult->num_rows > 0) {
                            while ($row = $requestResult->fetch_assoc()) {
                                echo "<tr>
                                    ...
                                    <td class='edit'>
                                        <a href='ccs-edit-venue.php?id=" . $row["id"] . "'>EDIT</a>
                                        <button class='delete-button' data-id='" . $row["id"] . "'>DELETE</button>
                                        <a href='ccs-venue-download.php?id=" . $row["id"] . "' class='download-button'>Download Report</a>
                                    </td>
                                </tr>";
                            }
                        } else {
            ...
            <div class="table-container">
                <table class="crud-table">
                    ...
                    <tbody>
                    <?php
                       $venueSql = "SELECT reservation_id, venue_name FROM ccs_venue_request ORDER BY reservation_id DESC";
            ...
            <div class="table-container">
                <table class="crud-table">
                    ...
                    <tbody>
                    <?php
                       $additionalSql = "SELECT reservation_id, additional_request, quantity FROM ccs_addedrequest ORDER BY reservation_id DESC";
           ...
           // Function to toggle the visibility of additional requests
            ...
            function showSuccessAlert() {
                Swal.fire({
                    ...
                    window.location.href = "ccs-resource.php"; // Redirect to the dashboard or desired page
                });
            }
            ...
            deleteButtons.forEach(button => {
                button.addEventListener("click", function (event) {
                    ...
                    fetch(`ccs-venue-delete.php?reservation_id=${reservationId}`)
