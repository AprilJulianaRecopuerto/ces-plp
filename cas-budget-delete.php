...
function sendNotificationAndEmail($projectName, $notificationMessage, $conn_mov) {
    ...
    if ($result->num_rows > 0) {
        ...
        try {
            ...
            $mail->Body = "<strong>$projectName</strong> $notificationMessage.<br><br>Best regards,<br>PLP CES";
            $mail->send();
        } catch (Exception $e) {
...
if ($action === 'count') {
    ...
    $countSql = "SELECT COUNT(*) as eventCount FROM cas_budget WHERE details_id = ?";
...
if ($action === 'all') {
    ...
    $deleteBudgetSql = "DELETE FROM cas_budget WHERE details_id = ?";
    $stmt = $conn->prepare($deleteBudgetSql);
    $stmt->bind_param('i', $detailsId);
    $response['success'] = $stmt->execute();

    if ($response['success']) {
        $deleteDetailsSql = "DELETE FROM cas_details WHERE id = ?";
        $stmt = $conn->prepare($deleteDetailsSql);
        $stmt->bind_param('i', $detailsId);
        $stmt->execute();

        // Send notification and email
        sendNotificationAndEmail("The CAS Project ID: $detailsId", "has been deleted completely", $conn_mov);
    }
} elseif ($action === 'specific' && $eventTitle) {
    // Delete a specific event by title
    $deleteBudgetSql = "DELETE FROM cas_budget WHERE details_id = ? AND event_title = ?";
    $stmt = $conn->prepare($deleteBudgetSql);
    $stmt->bind_param('is', $detailsId, $eventTitle);
    $response['success'] = $stmt->execute();

    $remainingEventsSql = "SELECT COUNT(*) as remainingCount FROM cas_budget WHERE details_id = ?";
    ...
    // Send notification and email
    sendNotificationAndEmail("The CAS event named '$eventTitle'", "has been deleted", $conn_mov);
    ...
    if ($result['eventCount'] === 1) {
        ...
        if ($response['success']) {
            ...
            // Send notification and email
            sendNotificationAndEmail("The CAS Project ID: $detailsId", "has been deleted as the only remaining event", $conn_mov);
        }
    }
}
