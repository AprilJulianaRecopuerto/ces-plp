...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'rename') {
    ...
    $check_sql = "SELECT * FROM cba_mov WHERE folder_name = '$new_folder_name'";
    $result = $conn_mov->query($check_sql);
    ...
    if (rename($old_folder_path, $new_folder_path)) {
        ...
        $sql = "UPDATE cba_mov SET folder_name = '$new_folder_name' WHERE folder_name = '$old_folder_name'";
        if ($conn_mov->query($sql) === TRUE) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'delete') {
    ...
    $recycle_bin_path = 'movuploads/cba-recycle/' . $folder_name; // Path to cba-recycle

    // Create cba-recycle directory if it doesn't exist
    if (!is_dir('movuploads/cba-recycle')) {
        mkdir('movuploads/cba-recycle', 0777, true);
    }

    // Move the folder to the cba-recycle
    if (is_dir($folder_path)) {
        if (rename($folder_path, $recycle_bin_path)) {
            $_SESSION['folder_delete_success'] = "Folder moved to recycle bin successfully.";
            // Delete the folder from the database
            $sql = "DELETE FROM cba_mov WHERE folder_name = '$folder_name'";
            if ($conn_mov->query($sql) !== TRUE) {
...
$result = $conn_mov->query("SELECT * FROM cba_mov");
$conn_mov->close();
?>
                    ...
                    if ($result && $result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo "<div class='folder' data-folder-name='$folder_name'>
                                    <a href='cba-upload.php?folder=" . urlencode($folder_name) . "' class='folder-link'>
           ...
           // Function to show success SweetAlert
            function showSuccessAlert(message) {
                Swal.fire({
                    ...
                    window.location.href = "cba-mov.php"; // Redirect to the desired page
                });
            }
