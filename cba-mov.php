...
$base_directory = 'movuploads/cba-mov/';

// Fetch the latest project title and dateof_imple from the proj_list database
$query = "SELECT proj_title, dateof_imple FROM cba ORDER BY id DESC LIMIT 1"; // Adjusted to fetch both columns
...
if ($result_proj->num_rows > 0) {
    ...
    // Folder path in 'movuploads/cba-mov/'
    $folder_path = $base_directory . $folder_name;

    // Check if the folder entry already exists in the cba_mov table
    $check_existing_folder_query = "SELECT * FROM cba_mov WHERE folder_name = '$folder_name'";
    ...
    if ($result_check->num_rows == 0) {
        ...
        if (!file_exists($folder_path)) {
            ...
            if (mkdir($folder_path, 0777, true)) {
                ...
                // Insert the folder name into the cba_mov table for tracking
                $insert_folder_sql = "INSERT INTO cba_mov (folder_name) VALUES ('$folder_name')";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'create') {
    ...
    $check_sql = "SELECT * FROM cba_mov WHERE folder_name = '$folder_name'";
    $result = $conn_mov->query($check_sql);

    if ($result->num_rows > 0) {
        $_SESSION['warning'] = 'The folder name already exists. Please choose a different name.';
        header('Location: cba-mov.php'); // Redirect after setting the session
        exit();
    }
    ...
    if (mkdir($folder_path, 0777, true)) {
        ...
        // Insert folder name into the cba_mov table
        $sql = "INSERT INTO cba_mov (folder_name) VALUES ('$folder_name')";
        if ($conn_mov->query($sql) === TRUE) {
            $_SESSION['folder_create_success'] = 'Folder and subfolders created successfully';
            header('Location: cba-mov.php'); // Redirect after success
            exit();
        } else {
            $_SESSION['folder_error'] = 'Error creating folder in database: ' . $conn_mov->error;
            header('Location: cba-mov.php'); // Redirect after error
            exit();
        }
    } else {
...
$query = "SELECT * FROM cba_mov WHERE folder_name IS NOT NULL"; // Adjusted to filter out deleted records
    ...
    <body>
        ...
        <div class="content-task">
            <div class="button-container">
                <a href="cba-archive.php" id="archive" class="archive-button">Archive</a>
            </div>
                    ...
                    if ($result && $result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo "<div class='folder' data-folder-name='$folder_name'>
                                    <a href='cba-subfolder.php?folder=" . urlencode($folder_name) . "' class='folder-link'>
        ...
        <script>
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                document.getElementById("archive").addEventListener("click", function(event) {
                    ...
                    logAndRedirect("Archive", "cba-archive.php");
                });
