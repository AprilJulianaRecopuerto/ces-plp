...
if (isset($_POST['delete_id'])) {
    ...
    $stmt->close();
    header("Location: cihm-task.php"); // Redirect back to project list page
    exit();
}
    ...
    <nav class="navbar">
        ...
        <div class="content-task">
            ...
            <div class="button-container">
                <button onclick="window.location.href='cihm-addtask.php'">Add</button>
            </div>
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                ...
                                foreach ($fileNames as $label => $fileName) {
                                    if (!empty($fileName)) { // Ensure the file name is not empty
                                        ...
                                        if ($fileExtension === 'pdf') {
                                            ...
                                            $fileLinks .= "<div style='margin-bottom: 10px;'><a href='cihm-task-fileviewer.php?file=" . urlencode($fileName) . "' target='_blank'>" . basename($fileName) . "</a></div>";
                                        } else {
                                            // For other file types, use JavaScript to handle downloads
                                            $fileLinks .= "<div style='margin-bottom: 10px;'><a href='cihm-task-fileviewer.php?file=" . urlencode($fileName) . "' class='download-file' data-filename='" . htmlspecialchars($fileName) . "'>" . basename($fileName) . "</a></div>";
                                        }
                                    }
                                }
                                ...
                                echo "<tr>
                                        ...
                                        <td class='edit'>
                                    <a href='cihm-edittask.php?id=" . htmlspecialchars($row["id"]) . "'>EDIT</a>
