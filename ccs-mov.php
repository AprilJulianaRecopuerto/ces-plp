...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'rename') {
    ...
    $check_sql = "SELECT * FROM ccs_mov WHERE folder_name = '$new_folder_name'";
    $result = $conn_mov->query($check_sql);
    ...
    if (rename($old_folder_path, $new_folder_path)) {
        ...
        $sql = "UPDATE ccs_mov SET folder_name = '$new_folder_name' WHERE folder_name = '$old_folder_name'";
        if ($conn_mov->query($sql) === TRUE) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'delete') {
    ...
    $recycle_bin_path = 'movuploads/ccs-recycle/' . $folder_name; // Path to ccs-recycle

    // Create ccs-recycle directory if it doesn't exist
    if (!is_dir('movuploads/ccs-recycle')) {
        mkdir('movuploads/ccs-recycle', 0777, true);
    }

    // Move the folder to the ccs-recycle
    if (is_dir($folder_path)) {
        if (rename($folder_path, $recycle_bin_path)) {
            $_SESSION['folder_delete_success'] = "Folder moved to recycle bin successfully.";
            // Delete the folder from the database
            $sql = "DELETE FROM ccs_mov WHERE folder_name = '$folder_name'";
            if ($conn_mov->query($sql) !== TRUE) {
...
$result = $conn_mov->query("SELECT * FROM ccs_mov");
$conn_mov->close();
?>
                    ...
                    if ($result && $result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo "<div class='folder' data-folder-name='$folder_name'>
                                    <a href='ccs-upload.php?folder=" . urlencode($folder_name) . "' class='folder-link'>
           ...
           // Function to show success SweetAlert
            function showSuccessAlert(message) {
                Swal.fire({
                    ...
                    window.location.href = "ccs-mov.php"; // Redirect to the desired page
                });
            }
