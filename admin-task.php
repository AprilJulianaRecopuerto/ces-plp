    ...
    <body>
        ...
        <div class="sidebar">
            ...
            <ul class="menu">
                ...
                <button class="dropdown-btn" class="active">
                    <img src="images/task.png">Task Management
                    <i class="fas fa-chevron-down" ></i> <!-- Dropdown icon -->
                </button>
                <div class="dropdown-container">
                    <a href="admin-task.php" class="active">Upload Files</a>
        ...
        <div class="content-task">
            ...
            foreach ($colleges as $college) {
                ...
                echo "<h3>" . strtoupper($college) . " - Task Management</h3>";

                // Fetch tasks for each college
                $taskTable = "{$college}"; // Assuming the task table is named like 'cas_tasks'
                $taskSql = "SELECT * FROM $taskTable";
                ...
                if ($taskResult && $taskResult->num_rows > 0) {
                    while ($row = $taskResult->fetch_assoc()) {
                        // Safely access the files with checks
                        $fileNames = [
                            'Letter Request:' => isset($row['letter_request']) ? htmlspecialchars($row['letter_request']) : '',
                            'Activity Design:' => isset($row['act_plan']) ? htmlspecialchars($row['act_plan']) : '',
                            'Terms of Reference:' => isset($row['termof_ref']) ? htmlspecialchars($row['termof_ref']) : '',
                            'Requisition Form:' => isset($row['requisition_form']) ? htmlspecialchars($row['requisition_form']) : '',
                            'Venue Reservation:' => isset($row['venue_reserve']) ? htmlspecialchars($row['venue_reserve']) : '',
                            'Budget Expenditure Plan:' => isset($row['budget_expenditure']) ? htmlspecialchars($row['budget_expenditure']) : '',
                        ];
                        ...
                        // Safely access date_of_implementation
                        $date_of_implementation = isset($row['date_of_implementation']) ? htmlspecialchars($row['date_of_implementation']) : 'N/A';
                    ...
                    echo "<tr><td colspan='6'>No tasks found for this college</td></tr>";
                }
