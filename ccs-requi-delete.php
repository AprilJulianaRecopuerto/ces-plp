...
if (!$itemName) {
    ...
    $countSql = "SELECT COUNT(*) as itemCount FROM ccs_items WHERE requisition_id = ?";
    ...
    if ($response['count'] === 1) {
        // Only one item, delete from ccs_items and then from ccs_requisition
        $deleteSql = "DELETE FROM ccs_items WHERE requisition_id = ?";
        $stmt = $conn->prepare($deleteSql);
        $stmt->bind_param('i', $requisitionId);
        $response['success'] = $stmt->execute();

        // If successful, delete from ccs_requisition
        if ($response['success']) {
            $deleteRequisitionSql = "DELETE FROM ccs_requisition WHERE requisition_id = ?";
    ...
    if ($itemName === 'all') {
        ...
        $deleteSql = "DELETE FROM ccs_items WHERE requisition_id = ? AND item_name = ?";
        $stmt = $conn->prepare($deleteSql);
        $stmt->bind_param('is', $requisitionId, $itemName);
        $response['success'] = $stmt->execute();

        // Check if there are remaining items for this requisition_id
        $remainingItemsSql = "SELECT COUNT(*) as remainingCount FROM ccs_items WHERE requisition_id = ?";
        $stmt = $conn->prepare($remainingItemsSql);
        $stmt->bind_param('i', $requisitionId);
        $stmt->execute();
        $remainingResult = $stmt->get_result()->fetch_assoc();

        // If no items remain, delete from ccs_requisition
