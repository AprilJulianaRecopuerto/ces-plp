...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn_proj_list->prepare("SELECT * FROM coed WHERE id = ?");
    ...
    if ($currentProject && (
        ...
        header("Location: coed-editproj.php?id=" . $projectId); // Redirect back with project ID
        exit();
    }

    // Prepare and bind the update statement
    $stmt = $conn_proj_list->prepare("UPDATE coed SET 
        date_of_sub=?, 
        semester=?, 
        lead_person=?, 
        dept=?, 
    ...
    if ($stmt->execute()) {
        ...
        if ($result->num_rows > 0) {
            ...
            try {
                ...
                header("Location: coed-editproj.php");
                exit;
            }
        } else {
        ...
        header("Location: coed-editproj.php?id=" . $projectId);
        exit();
    } else {
        $_SESSION['error'] = 'Failed to update project: ' . $stmt->error;
        header("Location: coed-editproj.php?id=" . $projectId); // Redirect with project ID
        exit();
    }
...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        header("Location: coed-projlist.php"); // Redirect to project list if no ID is found
        exit();
    }
} else {
    $_SESSION['error'] = 'No project ID specified.';
    header("Location: coed-projlist.php"); // Redirect to project list if no ID is specified
    exit();
}
                ...
                <form action="" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="lead_person" id="lead_person" required onchange="toggleOthersInput()">
                            <option value="COED Coordinator" <?php echo ($project['lead_person'] == 'COED Coordinator') ? 'selected' : ''; ?>>COED Coordinator</option>
                            <option value="Others" <?php echo ($project['lead_person'] != 'COED Coordinator') ? 'selected' : ''; ?>>Others (Please specify)</option>
                        </select>
                    </div>

                    <!-- Conditional input field for "Others" -->
                    <div class="form-group" id="othersInputGroup" style="display: <?php echo ($project['lead_person'] != 'COED Coordinator') ? 'block' : 'none'; ?>;">
                        <input type="text" name="other_lead_person" id="other_lead_person" value="<?php echo htmlspecialchars($project['lead_person'] != 'COED Coordinator' ? $project['lead_person'] : ''); ?>">
                    </div>
            ...
            document.addEventListener('DOMContentLoaded', (event) => {
                ...
                function showSuccessAlert(message) {
                    Swal.fire({
                        ...
                        window.location.href = "coed-projlist.php"; // Redirect to the project list
                    });
                }
                ...
                function showWarningAlert(message) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            window.location.href = 'coed-projlist.php';
                        }
