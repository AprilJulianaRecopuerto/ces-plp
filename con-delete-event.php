...
if (!$date) {
    ...
    if ($response['eventCount'] === 1) {
        // Prepare to delete from con_event_details
        $deleteSql = "DELETE FROM con_food WHERE con_tor_id = ?";
        $stmt = $conn->prepare($deleteSql);
        $stmt->bind_param('i', $eventFormId);
        
        // Execute deletion from con_event_details
        $response['success'] = $stmt->execute(); // Check if the deletion was successful
    
        // If successful, delete from con_event_form
        if ($response['success']) {
            $deleteFormSql = "DELETE FROM con_tor WHERE id = ?";
    ...
    if ($date === 'all') {
        ...
        $deleteSql = "DELETE FROM con_food WHERE con_tor_id = ? LIMIT 1";
        $stmt = $conn->prepare($deleteSql);
        $stmt->bind_param('i', $eventFormId);
    } else {
        // Delete events for a specific date
        $deleteSql = "DELETE FROM con_food WHERE con_tor_id = ? AND event_date = ?";
        $stmt = $conn->prepare($deleteSql);
    ...
    // If all events are deleted, delete the corresponding con_event_form record
