...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare to insert into ccs_details table
    $detailsStmt = $conn->prepare("INSERT INTO ccs_details (semester, district, barangay) VALUES (?, ?, ?)");
    if (!$detailsStmt) {
        die("Prepare failed: " . $conn->error);
    }
    $detailsStmt->bind_param("sss", $semester, $district, $barangay);

    // Execute the statement for ccs_details
    if ($detailsStmt->execute()) {
        ...
        foreach ($event_titles as $index => $event_title) {
            ...
            $total_expenses_query = $conn->prepare("SELECT SUM(expenses) AS total_expenses FROM ccs_budget WHERE details_id = ?");
            ...
            // Prepare to insert into ccs_budget table
            $budgetStmt = $conn->prepare("INSERT INTO ccs_budget (details_id, event_title, total_budget, expenses, remaining_budget) VALUES (?, ?, ?, ?, ?)");
            if (!$budgetStmt) {
                die("Prepare failed: " . $conn->error);
            }
            // Insert the current event budget entry into ccs_budget table
        ...
        $notification_message = "A new budget entry has been submitted for project: $event_title, $district, $barangay, by College of Computer Studies";
        ...
        if ($conn_mov->query($notification_sql) === TRUE) {
            ...
            if ($result->num_rows > 0) {
                ...
                try {
                    ...
                    header("Location: ccs-add-budget.php");
                    exit;
                }
            } else {
    ...
    header("Location: ccs-add-budget.php"); // Adjust this to your needs
    exit();
}
?>
        ...
        function removeEntry(element) {
            ...
            <?php if (isset($_SESSION['error'])): ?>
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = 'ccs-add-budget.php';
                    }
                });
