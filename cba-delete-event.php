...
if (!$date) {
    ...
    if ($response['eventCount'] === 1) {
        // Prepare to delete from cba_event_details
        $deleteSql = "DELETE FROM cba_food WHERE cba_tor_id = ?";
        $stmt = $conn->prepare($deleteSql);
        $stmt->bind_param('i', $eventFormId);
        
        // Execute deletion from cba_event_details
        $response['success'] = $stmt->execute(); // Check if the deletion was successful
    
        // If successful, delete from cba_event_form
        if ($response['success']) {
            $deleteFormSql = "DELETE FROM cba_tor WHERE id = ?";
    ...
    if ($date === 'all') {
        ...
        $deleteSql = "DELETE FROM cba_food WHERE cba_tor_id = ? LIMIT 1";
        $stmt = $conn->prepare($deleteSql);
        $stmt->bind_param('i', $eventFormId);
    } else {
        // Delete events for a specific date
        $deleteSql = "DELETE FROM cba_food WHERE cba_tor_id = ? AND event_date = ?";
        $stmt = $conn->prepare($deleteSql);
    ...
    // If all events are deleted, delete the corresponding cba_event_form record
