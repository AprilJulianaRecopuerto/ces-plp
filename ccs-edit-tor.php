...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM ccs_tor WHERE id = $eventId";
    ...
    $detailsSql = "SELECT * FROM ccs_food WHERE ccs_tor_id = $eventId";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Get posted data for ccs_event_form
    ...
    if (isset($_GET['id'])) {
        ...
        $updateEventSql = "UPDATE ccs_tor SET 
        ...
        if ($conn->query($updateEventSql) === TRUE) {
            ...
            $deleteDetailsSql = "DELETE FROM ccs_food WHERE ccs_tor_id = $eventId";
            ...
            foreach ($event_dates as $index => $date) {
                ...
                $insertDetailsSql = "INSERT INTO ccs_food (ccs_tor_id, event_date, event_title, meal_type, menu, total_meals, total_usage) VALUES ($eventId, '$date', '$title', '$mealType', '$menuItem', $meals, $usage)";
                if (!$conn->query($insertDetailsSql)) {
                    $_SESSION['error'] = 'Error inserting event detail: ' . $conn->error;
                    header("Location: ccs-edit-tor.php?id=$eventId");
                    exit;
                }
            }
            ...
            header("Location: ccs-edit-tor.php?id=$eventId"); // Redirect back to the edit page
            exit;
        } else {
            ...
            <div class="form-container">
                ...
                <form id="eventForm" action="" method="POST"> <!-- Change action as needed -->
                    <div class="form-group">
                        ...
                        <input type="text" id="college_name" name="college_name" value="<?php echo htmlspecialchars($eventFormData['college_name'] ?? ''); ?>" >
                    </div>
           ...
           // Set initial event count based on existing event sections
            ...
            function showSuccessAlert(message) {
                Swal.fire({
                    ...
                    window.location.href = "ccs-tor.php"; // Redirect to ccs-tor.php
                });
            }
