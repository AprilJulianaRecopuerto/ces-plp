...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT project_name, department, due_date, letter_request, act_plan, termof_ref, venue_reserve FROM ccs WHERE id = ?");
    ...
    if ($stmt->execute()) {
        ...
        // Compare existing data with the new data and create a notification message
        $notification_message = "Project update: ";

        if ($project_name !== $existingProject['project_name']) {
            $notification_message .= "Project Name changed from '{$existingProject['project_name']}' to '$project_name'. ";
        }

        if ($department !== $existingProject['department']) {
            $notification_message .= "Department changed from '{$existingProject['department']}' to '$department'. ";
        }

        if ($due_date !== $existingProject['due_date']) {
            $notification_message .= "Due Date changed from '{$existingProject['due_date']}' to '$due_date'. ";
        }

        if ($letter_request !== $existingProject['letter_request']) {
            $notification_message .= "Letter Request changed. ";
        }

        if ($act_plan !== $existingProject['act_plan']) {
            $notification_message .= "Action Plan changed. ";
        }

        if ($termof_ref !== $existingProject['termof_ref']) {
            $notification_message .= "Terms of Reference changed. ";
        }

        if ($venue_reserve !== $existingProject['venue_reserve']) {
            $notification_message .= "Venue Reservation changed. ";
        }
        ...
        $stmt_notification->bind_param('sss', $project_name, $department, $notification_message);
        ...
        if ($result->num_rows > 0) {
            ...
            try {
                ...
                $mail->Body    = "A task has been edited by <strong>$department</strong>.<br>Project Name: $project_name<br>Due Date: $due_date<br><br>Best regards,<br>PLP CES";
        ...
        header("Location: ccs-edittask.php?id=" . $projectId);
        exit();
    } else {
        $_SESSION['error'] = 'Failed to update project: ' . $stmt->error;
        header("Location: ccs-edittask.php?id=" . $projectId); // Redirect with project ID
        exit();
    }
...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION['error'] = 'Project not found.';
        header("Location: ccs-task.php"); // Redirect to project list if no ID is found
        exit();
    }
} else {
    $_SESSION['error'] = 'No project ID specified.';
    header("Location: ccs-task.php"); // Redirect to project list if no ID is specified
    exit();
}
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['letter_request'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $letterFileExtension === 'pdf' ? 'ccs-task-fileviewer.php?file=' . urlencode($project['letter_request']) : $letterFilePath; ?>" 
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['act_plan'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $actPlanFileExtension === 'pdf' ? 'ccs-task-fileviewer.php?file=' . urlencode($project['act_plan']) : $actPlanFilePath; ?>" 
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['termof_ref'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $termOfRefFileExtension === 'pdf' ? 'ccs-task-fileviewer.php?file=' . urlencode($project['termof_ref']) : $termOfRefFilePath; ?>" 
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['venue_reserve'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $venueReserveFileExtension === 'pdf' ? 'ccs-task-fileviewer.php?file=' . urlencode($project['venue_reserve']) : $venueReserveFilePath; ?>" 
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            function showSuccessAlert(message) {
                Swal.fire({
                    ...
                    window.location.href = "ccs-task.php"; // Redirect to the project list
                });
            }
