...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT project_name, department, due_date, letter_request, act_plan, termof_ref, venue_reserve FROM cba WHERE id = ?");
    ...
    if ($stmt->execute()) {
        ...
        header("Location: cba-edittask.php?id=" . $projectId);
        exit();
    } else {
        $_SESSION['error'] = 'Failed to update project: ' . $stmt->error;
        header("Location: cba-edittask.php?id=" . $projectId); // Redirect with project ID
        exit();
    }
...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION['error'] = 'Project not found.';
        header("Location: cba-task.php"); // Redirect to project list if no ID is found
        exit();
    }
} else {
    $_SESSION['error'] = 'No project ID specified.';
    header("Location: cba-task.php"); // Redirect to project list if no ID is specified
    exit();
}
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['letter_request'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $letterFileExtension === 'pdf' ? 'cba-task-fileviewer.php?file=' . urlencode($project['letter_request']) : $letterFilePath; ?>" 
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['act_plan'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $actPlanFileExtension === 'pdf' ? 'cba-task-fileviewer.php?file=' . urlencode($project['act_plan']) : $actPlanFilePath; ?>" 
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['termof_ref'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $termOfRefFileExtension === 'pdf' ? 'cba-task-fileviewer.php?file=' . urlencode($project['termof_ref']) : $termOfRefFilePath; ?>" 
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($project['venue_reserve'])) : ?>
                            <p>Current File: 
                                ...
                                ?>
                                <a href="<?php echo $venueReserveFileExtension === 'pdf' ? 'cba-task-fileviewer.php?file=' . urlencode($project['venue_reserve']) : $venueReserveFilePath; ?>" 
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            function showSuccessAlert(message) {
                Swal.fire({
                    ...
                    window.location.href = "cba-task.php"; // Redirect to the project list
                });
            }
