...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['reservation_id'])) {
    ...
    $countQuery = "
        SELECT 
            (SELECT COUNT(*) FROM coe_venue_request WHERE reservation_id = ?) AS venue_count,
            (SELECT COUNT(*) FROM coe_addedrequest WHERE reservation_id = ?) AS request_count
    ";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reservation_id'])) {
    ...
    // coee 1: Delete all records when both venue_name and request_name are 'all'
    if ($venue_name === 'all' && $request_name === 'all') {
        // Delete all venues
        $deleteAllVenuesQuery = "DELETE FROM coe_venue_request WHERE reservation_id = ?";
        ...
        $deleteAllRequestsQuery = "DELETE FROM coe_addedrequest WHERE reservation_id = ?";
        ...
        if ($success) {
            $deleteReservationQuery = "DELETE FROM coe_reservation WHERE id = ?";
            $stmt = $conn->prepare($deleteReservationQuery);
            $stmt->bind_param('i', $reservation_id);
            $stmt->execute();
        }
    } else if ($venue_name === 'single' && $request_name === 'single') {
        // coee 2: Delete single records when exactly 1 venue and 1 request exist
        $deleteSingleVenueQuery = "DELETE FROM coe_venue_request WHERE reservation_id = ? LIMIT 1";
        ...
        $deleteSingleRequestQuery = "DELETE FROM coe_addedrequest WHERE reservation_id = ? LIMIT 1";
        ...
        // coee 3: Delete specific venue or request
        // If a specific venue name is provided
        if ($venue_name) {
            $deleteVenueQuery = "DELETE FROM coe_venue_request WHERE reservation_id = ? AND venue_name = ?";
        ...
        if ($request_name) {
            $deleteRequestQuery = "DELETE FROM coe_addedrequest WHERE reservation_id = ? AND additional_request = ?";
