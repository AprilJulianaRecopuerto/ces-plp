...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['reservation_id'])) {
    ...
    $countQuery = "
        SELECT 
            (SELECT COUNT(*) FROM cihm_venue_request WHERE reservation_id = ?) AS venue_count,
            (SELECT COUNT(*) FROM cihm_addedrequest WHERE reservation_id = ?) AS request_count
    ";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reservation_id'])) {
    ...
    // cihme 1: Delete all records when both venue_name and request_name are 'all'
    if ($venue_name === 'all' && $request_name === 'all') {
        // Delete all venues
        $deleteAllVenuesQuery = "DELETE FROM cihm_venue_request WHERE reservation_id = ?";
        ...
        $deleteAllRequestsQuery = "DELETE FROM cihm_addedrequest WHERE reservation_id = ?";
        ...
        if ($success) {
            $deleteReservationQuery = "DELETE FROM cihm_reservation WHERE id = ?";
            $stmt = $conn->prepare($deleteReservationQuery);
            $stmt->bind_param('i', $reservation_id);
            $stmt->execute();
        }
    } else if ($venue_name === 'single' && $request_name === 'single') {
        // cihme 2: Delete single records when exactly 1 venue and 1 request exist
        $deleteSingleVenueQuery = "DELETE FROM cihm_venue_request WHERE reservation_id = ? LIMIT 1";
        ...
        $deleteSingleRequestQuery = "DELETE FROM cihm_addedrequest WHERE reservation_id = ? LIMIT 1";
        ...
        // cihme 3: Delete specific venue or request
        // If a specific venue name is provided
        if ($venue_name) {
            $deleteVenueQuery = "DELETE FROM cihm_venue_request WHERE reservation_id = ? AND venue_name = ?";
        ...
        if ($request_name) {
            $deleteRequestQuery = "DELETE FROM cihm_addedrequest WHERE reservation_id = ? AND additional_request = ?";
