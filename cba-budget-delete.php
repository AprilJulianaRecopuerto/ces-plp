...
if ($action === 'count') {
    ...
    $countSql = "SELECT COUNT(*) as eventCount FROM cba_budget WHERE details_id = ?";
...
if ($action === 'all') {
    ...
    $deleteBudgetSql = "DELETE FROM cba_budget WHERE details_id = ?";
    $stmt = $conn->prepare($deleteBudgetSql);
    $stmt->bind_param('i', $detailsId);
    $response['success'] = $stmt->execute();

    if ($response['success']) {
        $deleteDetailsSql = "DELETE FROM cba_details WHERE id = ?";
    ...
    $deleteBudgetSql = "DELETE FROM cba_budget WHERE details_id = ? AND event_title = ?";
    $stmt = $conn->prepare($deleteBudgetSql);
    $stmt->bind_param('is', $detailsId, $eventTitle);
    $response['success'] = $stmt->execute();

    $remainingEventsSql = "SELECT COUNT(*) as remainingCount FROM cba_budget WHERE details_id = ?";
