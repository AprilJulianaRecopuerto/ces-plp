...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Insert into cba_tor
    $stmt = $conn->prepare("INSERT INTO cba_tor (tor_sub, college_name, procurement_title, agency, date_of_delivery) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("issss", $tor_sub, $college_name, $procurement_title, $agency, $date_of_delivery);

    if ($stmt->execute()) {
        // Get the last inserted ID for cba_tor
        $cba_tor_id = $conn->insert_id;
        ...
        for ($i = 0; $i < count($event_dates); $i++) {
            ...
            error_log("Inserting: $cba_tor_id, $event_date, $event_title, $meal_type, $menu, $meal_count, $usage_count");
    
            // Insert into cba_food
            $food_stmt = $conn->prepare("INSERT INTO cba_food (cba_tor_id, event_date, event_title, meal_type, menu, total_meals, total_usage) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $food_stmt->bind_param("issssii", $cba_tor_id, $event_date, $event_title, $meal_type, $menu, $meal_count, $usage_count);
    
            // Execute and check for errors
            if (!$food_stmt->execute()) {
                error_log("Failed to insert into cba_food: " . $food_stmt->error);
            } else {
                error_log("Inserted successfully into cba_food");
            }
        }
        ...
        header("Location: cba-tor.php"); // Redirect back to the cba-tor page
        exit;
    }
        ...
        <script>
            ...
            function showSuccessAlert() {
                Swal.fire({
                    ...
                    window.location.href = "cba-tor.php"; // Redirect to the dashboard or desired page
                });
            }
