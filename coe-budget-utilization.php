...
if (isset($_POST['delete_id'])) {
    ...
    $stmt->close();
    header("Location: coe-budget-utilization.php"); // Redirect back to project list page
    exit();
}

// Fetch projects
$sql = "SELECT * FROM coe_budget";
...
<html lang="en">
    ...
    <nav class="navbar">
        ...
        <div class="content-budget">
            <h2>
                <span style="color: orange; font-size: 28px;"><?php echo htmlspecialchars($departmentName); ?></span> Budget
            </h2>

            <div class="button-container">
                <button onclick="window.location.href='coe-add-budget.php'">Add Budget</button>
            </div>
            ...
            <div class="table-container">
                <table class="crud-table">
                    ...
                    <tbody>
                        ...
                        // Count total records
                        $countSql = "SELECT COUNT(*) as total FROM coe_budget"; // Adjust to your actual table
                        $countResult = $conn->query($countSql);
                        $totalRecords = $countResult->fetch_assoc()['total'];
                        $totalPages = ceil($totalRecords / $limit); // Calculate total pages

                        // Fetch project data with pagination
                        $sql = "SELECT * FROM coe_budget LIMIT ?, ?";
                        $stmt = $conn->prepare($sql);
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo "<tr>
                                        ...
                                        <td class='edit'>
                                            <a href='coe-edit-budget.php?id={$row["id"]}'>EDIT</a>
