...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM coe_tor WHERE id = $eventId";
    ...
    $detailsSql = "SELECT * FROM coe_food WHERE coe_tor_id = $eventId";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Get posted data for coe_event_form
    ...
    if (isset($_GET['id'])) {
        ...
        $updateEventSql = "UPDATE coe_tor SET 
        ...
        if ($conn->query($updateEventSql) === TRUE) {
            ...
            $deleteDetailsSql = "DELETE FROM coe_food WHERE coe_tor_id = $eventId";
            ...
            foreach ($event_dates as $index => $date) {
                ...
                $insertDetailsSql = "INSERT INTO coe_food (coe_tor_id, event_date, event_title, meal_type, menu, total_meals, total_usage) VALUES ($eventId, '$date', '$title', '$mealType', '$menuItem', $meals, $usage)";
                if (!$conn->query($insertDetailsSql)) {
                    $_SESSION['error'] = 'Error inserting event detail: ' . $conn->error;
                    header("Location: coe-edit-tor.php?id=$eventId");
                    exit;
                }
            }
            ...
            header("Location: coe-edit-tor.php?id=$eventId"); // Redirect back to the edit page
            exit;
        } else {
           ...
           // Set initial event count based on existing event sections
            ...
            function showSuccessAlert(message) {
                Swal.fire({
                    ...
                    window.location.href = "coe-tor.php"; // Redirect to ccs-tor.php
                });
            }
