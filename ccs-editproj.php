...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn_proj_list->prepare("SELECT * FROM ccs WHERE id = ?");
    ...
    $stmt = $conn_proj_list->prepare("UPDATE ccs SET 
        date_of_sub=?, 
        semester=?, 
        lead_person=?, 
        dept=?, 
    ...
    if ($stmt->execute()) {
        ...
        if ($result->num_rows > 0) {
            ...
            try {
                ...
                header("Location: ccs-editproj.php");
                exit;
            }
        } else {
    ...
    <body>
        ...
        <div class="sidebar">
            ...
            <ul class="menu">
                ...
                <li><a href="ccs-projlist.php" class="active"><img src="images/project-list.png">Project List</a></li>
                ...
                <form action="" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="lead_person" id="lead_person" required onchange="toggleOthersInput()">
                            <option value="CCS Coordinator" <?php echo ($project['lead_person'] == 'CCS Coordinator') ? 'selected' : ''; ?>>CCS Coordinator</option>
                            <option value="Others" <?php echo ($project['lead_person'] != 'CCS Coordinator') ? 'selected' : ''; ?>>Others (Please specify)</option>
                        </select>
                    </div>

                    <!-- Conditional input field for "Others" -->
                    <div class="form-group" id="othersInputGroup" style="display: <?php echo ($project['lead_person'] != 'CCS Coordinator') ? 'block' : 'none'; ?>;">
                        <input type="text" name="other_lead_person" id="other_lead_person" value="<?php echo htmlspecialchars($project['lead_person'] != 'CCS Coordinator' ? $project['lead_person'] : ''); ?>">
                    </div>
