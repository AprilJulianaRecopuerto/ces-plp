...
$query = "SELECT * FROM coe_budget WHERE id = ?";
...
if (!$project) {
    ...
    header("Location: coe-edit-budget.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($semester === $project['semester'] &&
        ...
        header("Location: coe-edit-budget.php?id=" . $project_id); // Redirect with project ID
        exit();
    }
    ...
    $query = "UPDATE coe_budget SET semester = ?, project_title = ?, district = ?, barangay = ?, total_expenses = ?, remaining_balance = ? WHERE id = ?";
    $stmt = $conn->prepare($query);
    ...
    if ($stmt->execute()) {
        ...
        $query = "SELECT id, total_budget, remaining_balance FROM coe_budget WHERE id > ? ORDER BY id ASC";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $project_id);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        while ($subsequent_project = $result->fetch_assoc()) {
            ...
            $query = "UPDATE coe_budget SET total_budget = ? WHERE id = ?";
            $stmt = $conn->prepare($query);
        ...
        $query = "DELETE FROM coe_budget WHERE id > ? AND remaining_balance <= 0";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $project_id);
        $stmt->execute();

        $_SESSION['success'] = "Project updated successfully.";
        header("Location: coe-edit-budget.php?id=" . $project_id);
        exit();
    } else {
    ...
    <nav class="navbar">
        ...
        <div class="sidebar">
            ...
            <ul class="menu">
                ...
                <li><a href="coe-budget-utilization.php" class="active"><img src="images/budget.png">Budget Allocation</a></li>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', (event) => {
                ...
                Swal.fire({
                    ...
                    window.location.href = "coe-budget-utilization.php"; // Redirect to the project list
                });
            }
            ...
            function showWarningAlert(message) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        window.location.href = 'coe-budget-utilization.php';
                    }
