    ...
    <body>
        ...
        <div class="content-tor">
            <div class="button-container">
                <button onclick="logAndRedirect('Add TOR','ccs-add-resource.php')">TOR Form</button>
            </div>
                ...
                <div class="table-container">
                    <table class="crud-table">
                        ...
                        <tbody>
                            ...
                            // Count total records
                            $countSql = "SELECT COUNT(*) as total FROM ccs_tor";
                            $countResult = $conn->query($countSql);
                            $totalRecords = $countResult->fetch_assoc()['total'];
                            $totalPages = ceil($totalRecords / $limit); // Calculate total pages

                            // Fetch paginated event form details
                            $eventFormSql = "SELECT * FROM ccs_tor ORDER BY id LIMIT $limit OFFSET $offset";
                            ...
                            if ($resultEventForm && $resultEventForm->num_rows > 0) {
                                while ($row = $resultEventForm->fetch_assoc()) {
                                    echo "<tr>
                                        ...
                                        <td class='edit'>
                                            <a href='ccs-edit-tor.php?id=" . $row["id"] . "'>EDIT</a>
                                            <button class='delete-button' data-id='" . $row["id"] . "'>DELETE</button>
                                            <a href='ccs-download-report.php?id=" . $row["id"] . "' class='download-button'>Download Report</a>
                                        </td>
                                    </tr>";
                                }
                            } else {
                ...
                <div class="table-container">
                    <table class="crud-table">
                        ...
                        <tbody>
                            <?php
                            // Fetch event details
                            $eventDetailsSql = "SELECT * FROM ccs_food ORDER BY ccs_tor_id, event_date"; // Ensure the table name is correct
                            ...
                            if ($resultEventDetails && $resultEventDetails->num_rows > 0) {
                                ...
                                while ($row = $resultEventDetails->fetch_assoc()) {
                                    $eventData[$row['ccs_tor_id']][$row['event_date']][] = $row;
                                }
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    deleteButtons.forEach(button => {
                        button.addEventListener('click', function() {
                            ...
                            fetch(`ccs-delete-event.php?ccs_tor_id=${eventId}`)
                    ...
                    function deleteEvent(eventId, date) {
                        fetch(`ccs-delete-event.php?ccs_tor_id=${eventId}&event_date=${date}`, {
                            method: 'POST'
                        })
                        .then(response => response.json())
                        .then(data => {
