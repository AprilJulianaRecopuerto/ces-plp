...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM cihm_reservation WHERE id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $sqlVenue = "SELECT venue_name FROM cihm_venue_request WHERE reservation_id = ?";
        ...
        $sqlAdditional = "SELECT additional_request, quantity FROM cihm_addedrequest WHERE reservation_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_GET['id'])) {
        ...
        $updateReservationSql = "UPDATE cihm_reservation SET 
        ...
        if ($conn->query($updateReservationSql) === TRUE) {
            ...
            $deleteVenuesSql = "DELETE FROM cihm_venue_request WHERE reservation_id = $reservationId";
            ...
            foreach ($newVenueRequests as $venue) {
                if (!empty($venue)) {
                    $insertVenueSql = "INSERT INTO cihm_venue_request (reservation_id, venue_name) 
            ...
            $deleteAdditionalSql = "DELETE FROM cihm_addedrequest WHERE reservation_id = $reservationId";
            ...
            if (!empty($newAdditionalRequests) && is_array($newAdditionalRequests)) {
                foreach ($newAdditionalRequests as $request) {
                    if (!empty($request)) {
                        ...
                        $insertAdditionalSql = "INSERT INTO cihm_addedrequest (reservation_id, additional_request, quantity) VALUES (?, ?, ?)";
            ...
            <div class="form-container">
                ...
                <form id="venueForm" action="" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="college_name" name="college_name" value="<?php echo htmlspecialchars($requisitionFormData['college_name'] ?? 'College of International Hospitality Management'); ?>" placeholder="Enter College Name" required>
                    </div>
        ...
        <script>
            ...
            function showSuccessAlert(message) {
                Swal.fire({
                    ...
                    window.location.href = "cihm-venue.php"; // Redirect to cihm-resource.php
                });
            }
