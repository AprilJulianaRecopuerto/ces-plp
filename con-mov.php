...
$query = "SELECT proj_title, dateof_imple FROM con ORDER BY id DESC LIMIT 1"; // Adjusted to fetch both columns
...
if ($result_proj->num_rows > 0) {
    ...
    // Folder path in 'movuploads/con-mov/'
    $folder_path = $base_directory . $folder_name;

    // Check if the folder entry already exists in the con_mov table
    $check_existing_folder_query = "SELECT * FROM con_mov WHERE folder_name = '$folder_name'";
    ...
    if ($result_check->num_rows == 0) {
        ...
        if (!file_exists($folder_path)) {
            ...
            if (mkdir($folder_path, 0777, true)) {
                ...
                // Insert the folder name into the con_mov table for tracking
                $insert_folder_sql = "INSERT INTO con_mov (folder_name) VALUES ('$folder_name')";
...
$query = "SELECT * FROM con_mov WHERE folder_name IS NOT NULL"; // Adjusted to filter out deleted records
                    ...
                    if ($result && $result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo "<div class='folder' data-folder-name='$folder_name'>
                                    <a href='con-subfolder.php?folder=" . urlencode($folder_name) . "' class='folder-link'>
        ...
        <script>
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                document.getElementById("archive").addEventListener("click", function(event) {
                    ...
                    logAndRedirect("Archive", "con-archive.php");
                });
