...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Insert into coed_tor
    $stmt = $conn->prepare("INSERT INTO coed_tor (tor_sub, college_name, procurement_title, agency, date_of_delivery) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("issss", $tor_sub, $college_name, $procurement_title, $agency, $date_of_delivery);

    if ($stmt->execute()) {
        // Get the last inserted ID for coed_tor
        $coed_tor_id = $conn->insert_id;
        ...
        for ($i = 0; $i < count($event_dates); $i++) {
            ...
            error_log("Inserting: $coed_tor_id, $event_date, $event_title, $meal_type, $menu, $meal_count, $usage_count");
    
            // Insert into coed_food
            $food_stmt = $conn->prepare("INSERT INTO coed_food (coed_tor_id, event_date, event_title, meal_type, menu, total_meals, total_usage) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $food_stmt->bind_param("issssii", $coed_tor_id, $event_date, $event_title, $meal_type, $menu, $meal_count, $usage_count);
    
            // Execute and check for errors
            if (!$food_stmt->execute()) {
                error_log("Failed to insert into coed_food: " . $food_stmt->error);
            } else {
                error_log("Inserted successfully into coed_food");
            }
        }
        ...
        header("Location: coed-tor.php"); // Redirect back to the coed-tor page
        exit;
    }
        ...
        <script>
            ...
            function showSuccessAlert() {
                Swal.fire({
                    ...
                    window.location.href = "coed-tor.php"; // Redirect to the dashboard or desired page
                });
            }
