...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Insert into ccs_tor
    $stmt = $conn->prepare("INSERT INTO ccs_tor (tor_sub, college_name, procurement_title, agency, date_of_delivery) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("issss", $tor_sub, $college_name, $procurement_title, $agency, $date_of_delivery);

    if ($stmt->execute()) {
        // Get the last inserted ID for ccs_tor
        $ccs_tor_id = $conn->insert_id;
        ...
        for ($i = 0; $i < count($event_dates); $i++) {
            ...
            error_log("Inserting: $ccs_tor_id, $event_date, $event_title, $meal_type, $menu, $meal_count, $usage_count");
    
            // Insert into ccs_food
            $food_stmt = $conn->prepare("INSERT INTO ccs_food (ccs_tor_id, event_date, event_title, meal_type, menu, total_meals, total_usage) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $food_stmt->bind_param("issssii", $ccs_tor_id, $event_date, $event_title, $meal_type, $menu, $meal_count, $usage_count);
    
            // Execute and check for errors
            if (!$food_stmt->execute()) {
                error_log("Failed to insert into ccs_food: " . $food_stmt->error);
            } else {
                error_log("Inserted successfully into ccs_food");
            }
        }
        ...
        header("Location: ccs-tor.php"); // Redirect back to the ccs-tor page
        exit;
    }
           ...
           let inactivityTime = function () {
            ...
            function showSuccessAlert() {
                Swal.fire({
                    ...
                    window.location.href = "ccs-tor.php"; // Redirect to the dashboard or desired page
                });
            }
