...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO cba (date_of_sub, semester, lead_person, dept, implementor, proj_title, classification, specific_activity, dateof_imple, time_from, time_to, district, barangay, beneficiary, duration, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    if ($stmt === false) {
        $_SESSION['error'] = "Prepare statement failed: " . $conn->error;
        $redirect_url = 'cba-addproj.php';
    } else {
        // Bind parameters
        ...
        if ($stmt->execute()) {
            ...
            $redirect_url = 'cba-projlist.php'; // Redirect URL
            ...
            $redirect_url = 'cba-addproj.php'; // Redirect URL
        }
...
// Redirect the user to the appropriate page based on success or error
?>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function showSuccessAlert() {
                Swal.fire({
                    ...
                    window.location.href = "cba-projlist.php"; // Redirect to the dashboard or desired page
                });
            }
