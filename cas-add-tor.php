...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->execute()) {
        ...
        for ($i = 0; $i < count($event_dates); $i++) {
            ...
            error_log("Inserting: $cas_tor_id, $event_date, $event_title, $meal_type, $menu, $meal_count, $usage_count");
    
            // Insert into cas_food
            $food_stmt = $conn->prepare("INSERT INTO cas_food (cas_tor_id, event_date, event_title, meal_type, menu, total_meals, total_usage) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $food_stmt->bind_param("issssii", $cas_tor_id, $event_date, $event_title, $meal_type, $menu, $meal_count, $usage_count);
    
            // Execute and check for errors
            if (!$food_stmt->execute()) {
                error_log("Failed to insert into cas_food: " . $food_stmt->error);
            } else {
                error_log("Inserted successfully into cas_food");
            }
        }
        ...
        header("Location: cas-tor.php"); // Redirect back to the cas-tor page
        exit;
    }
        ...
        <script>
            ...
            function showSuccessAlert() {
                Swal.fire({
                    ...
                    window.location.href = "cas-tor.php"; // Redirect to the dashboard or desired page
                });
            }
